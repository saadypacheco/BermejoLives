-- La miniatura al buscador: la pantalla venía bajando las fotos grandes.
--
-- `buscar_comercios` devolvía `portada_url` y no `portada_thumb_url`, así que
-- las tarjetas de resultados —treinta por página— bajaban la versión de 1280px
-- (140 a 330 KB cada una) para dibujarla a 300px de ancho. En una búsqueda son
-- varios megabytes de datos móviles para ver lo mismo.
--
-- La miniatura pesa alrededor de 20 KB. Es la diferencia entre que el buscador
-- ande o no ande con la señal de Bermejo, que es donde va a usarse.
--
-- El globo del mapa la necesita por lo mismo: en el celular el globo es TODO lo
-- que se ve de un comercio al tocar su pin, así que ahora lleva foto.
--
