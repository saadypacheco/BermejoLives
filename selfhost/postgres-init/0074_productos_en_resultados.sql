-- El buscador no podía decir qué vende cada local, teniendo el dato.
--
-- `prod_obs_human` (lo que anotó el agente) y `prod_det_ia` (lo que la IA leyó
-- en la vidriera) están en `comercios` desde la 0047 y alimentan el índice de
-- búsqueda: por eso el local APARECE al buscar "zapatillas". Pero
-- `buscar_comercios` nunca los devolvió, así que la tarjeta mostraba un rubro
-- amplio —"Calzado"— y el comprador tenía que confiar en que adentro había lo
-- que buscaba.
--
-- Es la misma clase de falla que ya apareció con `horario`: el dato viaja hasta
-- el borde de la pantalla y ahí se tira. No rompe nada, no aparece en ningún
-- log, y sólo se nota si alguien va a mirar por qué la tarjeta dice poco.
--
-- Se BORRA y se recrea: cambia el tipo de retorno y `create or replace` no
-- puede con eso. `refinamientos_busqueda` la llama por nombre desde un cuerpo
-- de texto, así que no hay dependencia que el drop rompa.
