-- ============================================================
-- Bermejo Live Market · 0010_comercio_rubros
-- Un comercio puede tener VARIOS rubros (vende de todo). Tabla N:M.
-- comercios.rubro_id se mantiene como rubro PRINCIPAL (para mostrar).
-- ============================================================
create table if not exists comercio_rubros (
  comercio_id uuid not null references comercios(id) on delete cascade,
  rubro_id    uuid not null references rubros(id)    on delete cascade,
  primary key (comercio_id, rubro_id)
);

grant select on comercio_rubros to anon, authenticated;
grant all on comercio_rubros to service_role;
alter table comercio_rubros enable row level security;
drop policy if exists comercio_rubros_public_read on comercio_rubros;
create policy comercio_rubros_public_read on comercio_rubros
  for select to anon, authenticated using (true);

-- Backfill: cada comercio con rubro_id pasa a tener esa fila en la N:M
insert into comercio_rubros (comercio_id, rubro_id)
select id, rubro_id from comercios where rubro_id is not null
on conflict do nothing;

-- buscar_comercios: el filtro por rubro ahora matchea CUALQUIER rubro del comercio
